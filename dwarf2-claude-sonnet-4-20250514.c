...
static struct trie_node *alloc_trie_leaf(bfd *abfd)
{
  ...
  amt = sizeof(*leaf) + TRIE_LEAF_SIZE * sizeof(leaf->ranges[0]);
  ...
  if (contents == NULL)
      ...
      if (bfd_section_size_insane (abfd, msec))
	{
      ...
      *section_size = amt;
      ...
      if (amt == 0 || amt >= SIZE_MAX)
	{
	  bfd_set_error (bfd_error_no_memory);
	  return false;
	}

      contents = (bfd_byte *) bfd_malloc (amt + 1);
      if (contents == NULL)
	{
	  bfd_set_error (bfd_error_no_memory);
	  return false;
  ...
  if (offset >= *section_size)
    {
      _bfd_error_handler (_("DWARF error: offset (%" PRIu64 ")"
			    " greater than or equal to section size (%" PRIu64 ")"),
			  (uint64_t) offset, (uint64_t) *section_size);
      bfd_set_error (bfd_error_bad_value);
      return false;
    }
...
read_n_bytes (bfd *abfd, bfd_byte **ptr, bfd_byte *end, int n)
{
  ...
  if (!ptr || !*ptr || !end || !abfd || n <= 0)
    return 0;
  
  buf = *ptr;
  
  if (end < buf || (size_t)(end - buf) < (size_t)n)
    {
      *ptr = end;
      return 0;
    }
...
static int
read_1_signed_byte (bfd *abfd, bfd_byte **ptr, bfd_byte *end)
{
  bfd_byte *buf;
...
read_blk (bfd *abfd, bfd_byte **ptr, bfd_byte *end, size_t size)
{
  ...
  buf = *ptr;
  available_bytes = (size_t)(end - buf);
                      ...
                      bfd_byte *info_ptr_end)
{
  ...
  switch (form)
    ...
    case DW_FORM_ref_addr:
      if (unit->version >= 3)
        {
          attr->u.val = (unit->offset_size == 4) ?
            read_4_bytes (unit->abfd, &info_ptr, info_ptr_end) :
            read_8_bytes (unit->abfd, &info_ptr, info_ptr_end);
          break;
        }
    case DW_FORM_addr:
    ...
    case DW_FORM_strx1:
      attr->u.val = read_1_byte (abfd, &info_ptr, info_ptr_end);
      attr->u.str = (unit->dwarf_str_offset != 0) ?
        (char *) read_indexed_string (attr->u.val, unit) : NULL;
      break;
	       ...
	       int end_sequence)
{
  ...
  amt = sizeof (struct line_info);
  info = (struct line_info *) bfd_alloc (table->abfd, amt);
  if (!info)
    return false;
  ...
  if (!seq || seq->last_line->end_sequence)
    {
...
ranges_overlap (bfd_vma low1,
		...
		bfd_vma high2)
{
  bfd_vma min_low = (low1 <= low2) ? low1 : low2;
  bfd_vma max_low = (low1 <= low2) ? low2 : low1;
  bfd_vma min_high = (low1 <= low2) ? high1 : high2;
  ...
  if (trie->num_room_in_leaf > 0)
    {
      ...
      if (try_merge_with_existing_range(leaf, unit, low_pc, high_pc))
        return trie;

      if (can_insert_in_leaf(leaf))
        return insert_in_leaf(leaf, unit, low_pc, high_pc);

      if (should_split_leaf(leaf, trie_pc, trie_pc_bits))
        {
          trie = convert_leaf_to_interior(abfd, leaf, trie_pc, trie_pc_bits);
          if (!trie)
            return NULL;
        }
      else
        {
          trie = expand_leaf(abfd, leaf);
          if (!trie)
            return NULL;
          return insert_in_leaf((struct trie_leaf *) trie, unit, low_pc, high_pc);
        }
    }

  return insert_in_interior(abfd, trie, trie_pc, trie_pc_bits, unit, low_pc, high_pc);
}

static bool
try_merge_with_existing_range(struct trie_leaf *leaf, struct comp_unit *unit,
...
static bool
can_insert_in_leaf(const struct trie_leaf *leaf)
{
  return leaf->num_stored_in_leaf < leaf->head.num_room_in_leaf;
}

static struct trie_node *
insert_in_leaf(struct trie_leaf *leaf, struct comp_unit *unit,
               bfd_vma low_pc, bfd_vma high_pc)
{
...
static bool
should_split_leaf(const struct trie_leaf *leaf, bfd_vma trie_pc,
                  unsigned int trie_pc_bits)
{
...
static struct trie_node *
convert_leaf_to_interior(bfd *abfd, const struct trie_leaf *leaf,
                        bfd_vma trie_pc, unsigned int trie_pc_bits)
{
  struct trie_node *trie = bfd_zalloc(abfd, sizeof(struct trie_interior));
  if (!trie)
    return NULL;
...
static struct trie_node *
expand_leaf(bfd *abfd, const struct trie_leaf *leaf)
{
...
static struct trie_node *
insert_in_interior(bfd *abfd, struct trie_node *trie, bfd_vma trie_pc,
                  unsigned int trie_pc_bits, struct comp_unit *unit,
                  bfd_vma low_pc, bfd_vma high_pc)
{
...
compare_sequences(const void* a, const void* b)
{
    ...
    if (!seq1->last_line || !seq2->last_line) {
        return (seq1->last_line == seq2->last_line) ? 0 : (seq1->last_line ? 1 : -1);
    }
                      ...
                      struct line_sequence *seq)
{
  ...
  if (num_lines > SIZE_MAX / sizeof(struct line_info*))
    return false;
...
sort_line_sequences (struct line_info_table* table)
{
  ...
  if (amt / sizeof (struct line_sequence) != num_sequences)
    return false;
  ...
  seq = table->sequences;
  for (n = 0; n < num_sequences && seq != NULL; n++)
    {
  ...
  if ((table->num_dirs % DIR_ALLOC_CHUNK) == 0)
    {
      ...
      new_size = new_count * sizeof(char *);
      tmp = (char **) bfd_realloc (table->dirs, new_size);
      if (tmp == NULL)
        return false;
  ...
  if ((table->num_files % FILE_ALLOC_CHUNK) == 0)
    {
      ...
      tmp = (struct fileinfo *) bfd_realloc (table->files, new_size);
      if (!tmp)
	return false;
      table->files = tmp;
    }
...
read_formatted_entries (struct comp_unit *unit, bfd_byte **bufp,
			...
			bool (*callback) (struct line_info_table *table,
					  ...
					  unsigned int size))
{
  if (!unit || !bufp || !*bufp || !buf_end || !table || !callback) {
    bfd_set_error (bfd_error_bad_value);
    return false;
  }
  ...
  if (buf >= buf_end) {
    _bfd_error_handler (_("DWARF error: buffer exhausted"));
    bfd_set_error (bfd_error_bad_value);
    return false;
  }
...
decode_line_info (struct comp_unit *unit)
{
  ...
  size_t required_bytes = offset_size;
  if (lh.version >= 5)
    required_bytes += 8;
  ...
  if (line_ptr + required_bytes >= line_end)
    {
      _bfd_error_handler
				   ...
				   unsigned int *discriminator_ptr)
{
  ...
  *filename_ptr = NULL;
  ...
  if (mid + 1 < seq->num_lines && addr >= seq->line_info_lookup[mid + 1]->address)
    return false;
...
build_lookup_funcinfo_table(struct comp_unit *unit)
{
  ...
  lookup_funcinfo_table = unit->lookup_funcinfo_table;
  ...
  for (each = unit->function_table; each; each = each->prev_func)
    {
      ...
      low_addr = each->arange.low;
      high_addr = each->arange.high;

      for (range = each->arange.next; range; range = range->next)
        {
  ...
  qsort(lookup_funcinfo_table, number_of_functions,
        sizeof(struct lookup_funcinfo), compare_lookup_funcinfos);
				  ...
				  struct funcinfo **function_ptr)
{
  ...
  bfd_size_type first = find_first_function_containing_addr(unit, addr, number_of_functions);
  if (first >= number_of_functions)
    return false;

  struct funcinfo* best_fit = find_best_fit_function(unit, addr, first, number_of_functions);
  if (!best_fit)
    return false;
...
find_first_function_containing_addr(struct comp_unit *unit, bfd_vma addr, unsigned int number_of_functions)
{
  bfd_size_type low = 0;
...
find_best_fit_function(struct comp_unit *unit, bfd_vma addr, bfd_size_type first, unsigned int number_of_functions)
{
      ...
      for (arange = &each->arange; arange; arange = arange->next)
        {
          ...
          range_len = arange->high - arange->low;
          if (range_len >= best_fit_len)
            continue;
			...
			int *linenumber_ptr)
{
  ...
  if (attr_ptr->form == DW_FORM_ref_addr
      ...
      else
	{
	  ...
	  if (attr_ptr->form == DW_FORM_ref_addr && u == NULL)
	    {
	      while ((u = stash_comp_unit (unit->stash, &unit->stash->f)) != NULL)
		{
	  ...
	  if (attr_ptr->form == DW_FORM_GNU_ref_alt && u == NULL)
	    {
	      while ((u = stash_comp_unit (unit->stash, &unit->stash->alt)) != NULL)
		{
		  if (info_ptr >= u->info_ptr_unit && info_ptr < u->end_ptr)
		    break;
		  u = NULL;
	     ...
	     struct trie_node **trie_root, uint64_t offset)
{
  ...
  for (;;)
    {
      required_bytes = 2u * unit->addr_size;
      if (required_bytes > (size_t) (ranges_end - ranges_ptr))
	return false;
	       ...
	       struct trie_node **trie_root, uint64_t offset)
{
  ...
  if (!unit || !unit->file || !arange || !trie_root)
    return false;
		  ...
		  bool compunit)
{
  ...
  switch (attr->name)
    ...
    case DW_AT_comp_dir:
      process_comp_dir_attribute(unit, attr);
      break;
...
static void
process_comp_dir_attribute(struct comp_unit *unit, struct attribute *attr)
{
                            ...
                            unsigned int *discriminator_ptr)
{
    ...
    *function_ptr = NULL;
    ...
    bool func_found = lookup_address_in_function_table(unit, addr, function_ptr);
    
    if (func_found && *function_ptr && (*function_ptr)->tag == DW_TAG_inlined_subroutine) {
        if (unit->stash)
            unit->stash->inliner_chain = *function_ptr;
    }

    bool line_found = lookup_address_in_line_info_table(unit->line_table, addr,
                                                       filename_ptr,
                                                       linenumber_ptr,
                                                       discriminator_ptr);
    
    return line_found || func_found;
}
		 ...
		 asection *after_sec)
{
  ...
  if (after_sec == NULL)
    {
      msec = bfd_get_section_by_name (abfd, debug_sections[debug_info].uncompressed_name);
      if (msec != NULL && (msec->flags & SEC_HAS_CONTENTS) != 0)
	return msec;

      msec = bfd_get_section_by_name (abfd, debug_sections[debug_info].compressed_name);
      if (msec != NULL && (msec->flags & SEC_HAS_CONTENTS) != 0)
        return msec;
  ...
  for (msec = after_sec->next; msec != NULL; msec = msec->next)
	...
	continue;
      ...
      if (strcmp (msec->name, debug_sections[debug_info].uncompressed_name) == 0)
	return msec;

      if (debug_sections[debug_info].compressed_name != NULL 
          && strcmp (msec->name, debug_sections[debug_info].compressed_name) == 0)
	return msec;
...
unset_sections (struct dwarf2_debug *stash)
{
  ...
  for (int i = 0; i < stash->adjusted_section_count; i++, p++)
  {
    if (p->section)
...
place_sections (bfd *orig_bfd, struct dwarf2_debug *stash)
{
  ...
  int section_count = count_relevant_sections(orig_bfd, stash, debug_info_name);
  ...
  if (section_count <= 1)
    ...
    {
      if (!allocate_adjusted_sections(stash, section_count))
        return false;
      
      if (!adjust_section_addresses(orig_bfd, stash, debug_info_name))
        return false;
    }
...
static int
count_relevant_sections(bfd *orig_bfd, struct dwarf2_debug *stash, const char *debug_info_name)
{
  int count = 0;
  bfd *abfd = orig_bfd;
  ...
  do
    {
      for (asection *sect = abfd->sections; sect != NULL; sect = sect->next)
        {
          if (is_relevant_section(sect, abfd, orig_bfd, debug_info_name))
            count++;
        }
      abfd = (abfd == stash->f.bfd_ptr) ? NULL : stash->f.bfd_ptr;
    } while (abfd);
...
static bool
is_relevant_section(asection *sect, bfd *abfd, bfd *orig_bfd, const char *debug_info_name)
{
  ...
  return ((sect->flags & SEC_ALLOC) != 0 && abfd == orig_bfd) || is_debug_info;
}

static bool
allocate_adjusted_sections(struct dwarf2_debug *stash, int count)
{
  size_t amt = count * sizeof(struct adjusted_section);
  ...
  stash->adjusted_section_count = count;
  return true;
}

static bool
adjust_section_addresses(bfd *orig_bfd, struct dwarf2_debug *stash, const char *debug_info_name)
{
			      ...
			      bool do_place)
{
  ...
  if (!initialize_stash (abfd, stash, debug_sections, symbols))
    return false;
  ...
  if (msec == NULL && abfd == debug_bfd)
    {
      debug_bfd = try_open_debug_file (abfd, stash);
      if (!debug_bfd)
	return false;
      
      msec = find_debug_info (debug_bfd, debug_sections, NULL);
      if (!msec || !bfd_generic_link_read_symbols (debug_bfd))
	{
	  bfd_close (debug_bfd);
	  return false;
	}
  ...
  if (!load_debug_sections (debug_bfd, debug_sections, symbols, stash, msec))
    {
...
static bool
initialize_stash (bfd *abfd, struct dwarf2_debug *stash,
		  const struct dwarf_debug_section *debug_sections,
		  asymbol **symbols)
{
...
static bfd *
try_open_debug_file (bfd *abfd, struct dwarf2_debug *stash)
{
...
load_debug_sections (bfd *debug_bfd, const struct dwarf_debug_section *debug_sections,
		     asymbol **symbols, struct dwarf2_debug *stash, asection *msec)
{
  ...
  if (!find_debug_info (debug_bfd, debug_sections, msec))
    ...
    {
      if (!load_multiple_sections (debug_bfd, debug_sections, symbols, stash, &total_size))
	return false;
    }
...
static bool
load_multiple_sections (bfd *debug_bfd, const struct dwarf_debug_section *debug_sections,
  ...
  for (msec = find_debug_info (debug_bfd, debug_sections, NULL);
	...
	continue;
      ...
      if (!(bfd_simple_get_relocated_section_contents
	    (debug_bfd, msec, stash->f.dwarf_info_buffer + current_size,
	     symbols)))
	return false;
...
_bfd_dwarf2_find_nearest_line (bfd *abfd,
                               ...
                               void **pinfo)
{
  if (!abfd || !section || !filename_ptr || !functionname_ptr || 
   ...
   void **pinfo)
{
  ...
  if (!filename_ptr || !linenumber_ptr || !pinfo)
    return false;
  ...
  *linenumber_ptr = 0;
  ...
  if (!initialize_alt_file(stash, alt_filename))
    return false;

  if (!setup_address_and_section(&addr, &section, &do_line, symbol, offset, functionname_ptr))
    return false;

  if (!validate_section_address(section, &addr))
    return false;
  ...
  if (do_line) {
    ...
    found = search_nearest_line(stash, addr, filename_ptr, &function, linenumber_ptr, discriminator_ptr);
  }

  if (!found) {
    found = read_remaining_comp_units(stash, do_line, symbol, addr, filename_ptr, &function, linenumber_ptr, discriminator_ptr);
  }

  if (functionname_ptr) {
    handle_function_name_resolution(functionname_ptr, function, &found, stash, abfd, symbols, section, offset, filename_ptr);
  }
...
static bool
initialize_alt_file(struct dwarf2_debug *stash, const char *alt_filename)
{
...
static bool
setup_address_and_section(bfd_vma *addr, asection **section, bool *do_line, asymbol *symbol, bfd_vma offset, const char **functionname_ptr)
{
  *do_line = (symbol != NULL);
...
static bool
validate_section_address(asection *section, bfd_vma *addr)
{
  if (!section)
    return false;
...
static int
search_line_info(struct dwarf2_debug *stash, asymbol *symbol, bfd_vma addr, const char **filename_ptr, unsigned int *linenumber_ptr)
{
  if (stash->info_hash_status == STASH_INFO_HASH_OFF)
    stash_maybe_enable_info_hash_tables(stash->f.bfd_ptr, stash);

  if (stash->info_hash_status == STASH_INFO_HASH_ON)
    stash_maybe_update_info_hash_tables(stash);

  if (stash->info_hash_status == STASH_INFO_HASH_ON) {
    bool found = stash_find_line_fast(stash, symbol, addr, filename_ptr, linenumber_ptr);
    if (found)
      return true;
  }
...
static int
search_nearest_line(struct dwarf2_debug *stash, bfd_vma addr, const char **filename_ptr, struct funcinfo **function, unsigned int *linenumber_ptr, unsigned int *discriminator_ptr)
{
  ...
  if (trie && search_trie_leaf(trie, addr, filename_ptr, function, linenumber_ptr, discriminator_ptr))
    return true;

  return search_comp_units_without_ranges(stash, addr, filename_ptr, function, linenumber_ptr, discriminator_ptr);
}

static int
search_trie_leaf(struct trie_node *trie, bfd_vma addr, const char **filename_ptr, struct funcinfo **function, unsigned int *linenumber_ptr, unsigned int *discriminator_ptr)
{
  ...
  for (unsigned int i = 0; i < leaf->num_stored_in_leaf; ++i) {
    ...
    unit->mark = true;
    if (comp_unit_find_nearest_line(unit, addr, filename_ptr, function, linenumber_ptr, discriminator_ptr))
      return true;
  }
...
static int
search_comp_units_without_ranges(struct dwarf2_debug *stash, bfd_vma addr, const char **filename_ptr, struct funcinfo **function, unsigned int *linenumber_ptr, unsigned int *discriminator_ptr)
{
  ...
  for (struct comp_unit *each = *prev_each; each; each = each->next_unit_without_ranges) {
    ...
    if (comp_unit_find_nearest_line(each, addr, filename_ptr, function, linenumber_ptr, discriminator_ptr))
      return true;
...
static int
read_remaining_comp_units(struct dwarf2_debug *stash, bool do_line, asymbol *symbol, bfd_vma addr, const char **filename_ptr, struct funcinfo **function, unsigned int *linenumber_ptr, unsigned int *discriminator_ptr)
{
  struct comp_unit *each;
  ...
  while ((each = stash_comp_unit(stash, &stash->f)) != NULL) {
    ...
    if (do_line) {
      found = (((symbol->flags & BSF_FUNCTION) == 0 || comp_unit_may_contain_address(each, addr)) &&
...
static void
handle_function_name_resolution(const char **functionname_ptr, struct funcinfo *function, int *found, struct dwarf2_debug *stash, bfd *abfd, asymbol **symbols, asection *section, bfd_vma offset, const char **filename_ptr)
{
			...
			const char **functionname_ptr)
{
  ...
  if (cache_needs_update (cache, section, offset))
    {
      update_cache (cache, symbols, section, offset, abfd);
    }
...
static bool
cache_needs_update (elf_find_function_cache *cache, asection *section, bfd_vma offset)
{
  return cache->last_section != section
      || cache->func == NULL
...
static void
update_cache (elf_find_function_cache *cache, asymbol **symbols, asection *section, bfd_vma offset, bfd *abfd)
{
  asymbol *file = NULL;
  asymbol **p;
